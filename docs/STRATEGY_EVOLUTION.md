...
> Last regenerated: 2026-05-06. Items: 160 strategy rows (#1 → #253). Bug-fix rows are intentionally excluded; see `STRATEGY_ROADMAP.md` for the full Bug Fix log.
...
| 251a | Risk | Fractional-Kelly cap on the #251 surviving (un-paused) side (added 2026-05-06). When the #251 pause arms against one side, the OPPOSING side may have thin evidence (e.g. SELL n=14 in the trigger audit). Per Kelly criterion (typical lookback is 50-60 trades for win-prob estimation; binomial confidence interval at n=14 is ±26 percentage points — statistical noise) the surviving side is capped at 3 entries per session whenever its history has fewer than 20 trades. The bot used to lean its full per-session trade quota on the un-paused side just because the OTHER side broke down; now it reduces concentration on the un-validated side while letting it stay alive. |
