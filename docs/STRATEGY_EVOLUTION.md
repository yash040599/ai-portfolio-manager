...
| 32 | Execution | Late-entry targets are smaller (1 PM: −20%, 2 PM: −25%) since less time is left. |
...
| 230 | Bug Fix | Pre-trade gate-count drift — the docs claimed "41 checks" in 8 places (STRATEGY_V2 prose, README, code docstring) but the canonical inventory table only listed 40 rows after #225 collapsed two late-entry sub-gates. Reconciled all eight references to 40 and cleaned a stray table-cell artefact. Documentation accuracy only; no behavioural change. |
| 231 | Bug Fix | After a partial-profit exit the bot cancels the live exchange stop-loss and places a fresh one for the reduced quantity. If the cancel succeeded but the new placement failed (network / Kite glitch), the position was left with software-SL only AND no loud alert — EOD reports thought the exchange SL was still live. Now the same `*** EXCHANGE SL-M REPLACE FAILED ***` alert that fires on initial-entry failures (#152) also fires here, and the position is flagged so a bot restart cannot quietly resume thinking it is fully protected. |
