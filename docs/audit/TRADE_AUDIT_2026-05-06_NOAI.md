...
- Signal decay is mixed and should stay, but any change must run `scripts/trade/exit_coverage_check.py`.
...
- Keep `docs/TRADE_STRATEGY.md` as the strategy reference, but prune the giant
...
- Mark likely phase-1 scripts as deprecated if kept: `scripts/shared/view_analyses.py`
...
- Preserve `scripts/trade/rejection_audit.py`, `scripts/trade/verify_trades.py`,
  `scripts/trade/exit_coverage_check.py`, and `scripts/trade/strategy_stability_check.py`.

## What I Would Not Do Yet

- Do not raise `MIN_SCORE` blindly. Recent higher scores lost more.
