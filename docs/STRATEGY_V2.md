...
<!-- ══════════════════════════════════════════════════════════════
  ...
  Last sync: 2026-04-13 — Time-based R:R floor (morning 1.3, afternoon 1.2,
  late 1.0), config renamed POST_MERGE→RR_FLOOR, mid-day retry step-down.
══════════════════════════════════════════════════════════════ -->
...
| 3b | **R:R safety floor** | Time-based + adaptive | Morning (<1PM): 1.3:1. Afternoon (1-2PM): 1.2:1. Late (>2PM): 1.0:1. Relaxes to 1.1:1 after 3 failed scans, stops after 5. Mid-day retry: floor - 0.1 on mid-day rescans. |
...
| `RR_RELAX_AFTER_FAILS` | 3 | Scans before relaxing R:R floor |
...
| **R:R floor (time-based + adaptive)** | Explicit floor per time period: morning 1.3:1, afternoon 1.2:1, late 1.0:1. Relaxes to min(time_floor, 1.1) after 3 failed scans, stops after 5. Mid-day retry: floor - 0.1 on mid-day rescans. Config names: `RR_FLOOR_MORNING`, `RR_FLOOR_AFTERNOON`, `RR_FLOOR_LATE`, `RR_FLOOR_RELAXED`. | `config.py`, `order_engine.py`, `manager.py` |
...
| **R:R mid-day retry guard** | Mid-day retry step-down only fires on mid-day rescans (all-closed, slot-freed, opportunity). Morning scan excluded via `_initial_entry_done` flag — it has observation period + multiple candidates + adaptive relaxation. | `manager.py` |
