...
| **Gap Fade (weak-volume)** | **0.70** | **FAIL** | **Phase 9.9 (2026-06-12). SELL gap-ups on low volume (<1x avg), target prev close. 469 OOS trades, PF 0.70. Weak gaps don't reliably revert. RANGE-only PF 1.11 but thin.** |
| **EOD VWAP Reversion** | **0.55** | **FAIL** | **Phase 9.10 (2026-06-12). Enter at 14:30 when >1.5σ from VWAP, target VWAP by close. 472 OOS trades, WR 41.9%, PF 0.55. Extended stocks keep trending, don't revert.** |
| **Auction Alpha (candle shape)** | **0.74** | **FAIL** | **Phase 9.11 (2026-06-12). First candle body>60% + vol>1.5x as directional signal. 467 OOS trades, PF 0.74. Shape alone doesn't predict follow-through. Best param PF 0.87.** |
...
| 9.8 | **Budget scaling analysis** — would Rs.1L budget flip any failed strategy? | **NO.** Rs.1L saves only 0.024%/trade (0.106%→0.083%) because NSE charges are %-based, not flat. The Rs.20 brokerage cap only helps above ~Rs.67K/order. Only ORB-15 barely flips to breakeven (exp -0.010%→+0.014%, ~PF 1.01) — still below 1.15 gate. All others have 0.05-0.20% expectancy gaps that 0.024% can't close. **The problem is signal, not costs.** |
| 9.9 | **Backtest D.3 (Gap Fade)** — trade AGAINST weak-volume gaps (<1x avg vol). Fade gap-up on low volume → SELL, target prev close. Mirror of Gap-and-Go. NIFTY100, walk-forward OOS. | **DONE — FAIL. OOS PF 0.70.** 469 trades. Weak-volume gaps don't reliably mean-revert. RANGE-only shows PF 1.11 (213 trades) but too thin and below gate. No param combo (gap 0.3-1.5%, vol ceil 0.5-1.2x, sq-off 12-15) clears 1.0 on ALL regimes. ([scripts/trade/backtest_gap_fade.py](../scripts/trade/backtest_gap_fade.py)) |
| 9.10 | **Backtest D.4 (EOD VWAP Reversion)** — enter at 14:30 when stock >1.5σ from VWAP, target VWAP, exit by 15:10. Institutional VWAP-targeting in last 30 min. | **DONE — FAIL. OOS PF 0.55.** 472 trades, WR 41.9%. Stocks extended from VWAP at 14:30 do NOT revert by close — they keep trending. Earlier entry (13:30) slightly better PF 0.67 but still negative. No σ threshold (1.0-3.0) rescues it. ([scripts/trade/backtest_eod_vwap.py](../scripts/trade/backtest_eod_vwap.py)) |
| 9.11 | **Backtest D.5 (Auction Alpha)** — first candle shape (body >60% of range) + volume >1.5x as directional signal. "Poor man's order flow." | **DONE — FAIL. OOS PF 0.74.** 467 trades. Candle shape alone doesn't predict intraday follow-through. Best param combo (body≥0.5, vol≥2.0x) reaches PF 0.87. Gap confirm + RSI filter lifts to 0.82 but still <1.0. VOLATILE-only PF 0.54 (worse, not better). ([scripts/trade/backtest_auction_alpha.py](../scripts/trade/backtest_auction_alpha.py)) |
| 9.12 | **Phase 9 final verdict** | **20 strategies/variants tested across Phases 1-9. Only Gap-and-Go v1.1.1 passes the 1.15 OOS gate. The intraday equity search space on NSE is conclusively exhausted.** |
