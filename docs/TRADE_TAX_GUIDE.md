...
> dashboard — see [modes/dashboard/docs/DASHBOARD_ROADMAP.md](../modes/dashboard/docs/DASHBOARD_ROADMAP.md) item D18
...
2. **Build the intraday ledger:** `python scripts/trade/fill_intraday_ledger.py`
   — populates `intraday_tax_ledger` from live trading JSONs (charges + P&L per trade)
3. **Reconcile against Zerodha:** `python scripts/shared/import_zerodha_taxpnl.py --fy <YYYY>`
   — verifies/corrects ledger using the official Tax P&L sheet, populates `capital_gains_ledger`
4. **Get the FY summary:** `python scripts/shared/tax_summary.py --fy <YYYY>`
...
| Cumulative FY ledger | `python scripts/shared/tax_summary.py --fy <YYYY>` |
