...
| 147 | Session-time-aware RVol — hourly-bucket baseline, not daily average *(progress: 2026-04-21 added 22 afternoon RVol 0.6× rejections — NEUTRAL drift both ways. Rolling log of afternoon-RVol rejections growing; when we have ≥ 5 trading days of rejection samples, measure drift vs hypothetical hourly baseline)* | LOW | Medium | Medium |
| 24 | Backtesting framework — replay V2 scoring on historical data | LOW | Highest | High |

### Pending — Awaiting Trade Data (9 items)
...
| 190 | **Pattern-direction entry veto** (mirror of #174 SIGNAL_REVERSAL exit, applied at ENTRY). Currently candle patterns flow into `combined_score` as weighted contributions but never act as a hard veto at entry. The score absorbs them: a BUY can clear `|score| ≥ 6.0` even when one of the entry-tick patterns is on the bearish reversal set (`BEARISH_ENGULFING`, `EVENING_STAR`, `BEARISH_HARAMI`, `SHOOTING_STAR`, `HANGING_MAN`, `THREE_BLACK_CROWS`) — and vice-versa for SELL with the bullish set. Today: PNB BUY @ +6.1 with `BEARISH_ENGULFING` → STAGNANT -Rs.8.65; TRENT BUY @ +6.4 with `BEARISH_ENGULFING` + `StochRSI OVERBOUGHT` + `ADX 45` → STAGNANT -Rs.18.80. Both exited STAGNANT (small bleeders), exactly the failure mode #174 catches at exit but cheaper to prevent at entry. Proposed gate (in `enter_trade` after RSI checks, before ADX gate): if any entry-tick pattern is on the OPPOSITE-side reversal set AND `|score| < PATTERN_VETO_OVERRIDE_SCORE` (default 8.0 — same conviction floor as gap-coherence override #173), skip the entry with reason `"BUY pattern BEARISH_ENGULFING contradicts direction (score 6.4 < 8.0 override)"`. Single trading day's evidence; needs population. | After **≥ 10 entries** where the entry-tick pattern set contains an opposite-direction reversal pattern AND `|score|` 5.5-7.5, compare hit-rate / net R-multiple of those entries vs entries with no opposite pattern present. If opposite-pattern entries underperform clean entries by ≥ 30% hit-rate OR produce ≥ 2× the STAGNANT_EXIT rate, promote to main Pending and implement the entry-time veto (kill-switch via `PATTERN_VETO_ENABLED`). |
| 189 | **ADX-chop + borderline-score override floor tuning.** Rejection rule in `enter_trade` ([order_engine.py](../services/order_engine.py)) blocks entry when `ADX < ADX_MIN_THRESHOLD (18)` unless `|score| ≥ ADX_OVERRIDE_SCORE (7.0)`. NESTLEIND 2026-04-21 13:50 was rejected at `ADX 17.3, score 5.5` and went on to rip +4.18% / +Rs.650 (biggest single miss of the day by ~6×). The combination "ADX just below threshold (15-18 band) + score just below override (5-7 band) + GAP_UP_STRONG or equivalent tape confirmation" *may* be a systematically under-rated setup — or it may be one lucky outlier. Single data point. The right promotion path is not "lower the override" (that would let in real chop) but "add a tape-confirmation tier: if gap is `*_STRONG`, RVol ≥ 1.0×, AND ADX is trending up (rising over last 3 candles), allow entry at |score| ≥ 6.0 even on weak ADX". | After **≥ 10 "near-miss" rejections** (ADX 15.0-18.0 AND `|score|` 5.0-7.0), back-test verdicts: how many closed `MISSED_PROFIT` vs `AVOIDED_LOSS` vs `NEUTRAL` via the rejection audit. If ≥ 40% were `MISSED_PROFIT` AND missed profit totals ≥ 3× avoided losses, promote and implement the tape-confirmation tier. |
