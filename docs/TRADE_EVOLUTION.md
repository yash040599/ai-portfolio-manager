...
- **Bug fixes.** Crash fixes, exception handling, data-correctness patches — these belong only in commit messages and `TRADE_ROADMAP.md` (where they keep their `Bug Fix` category). Skip them in this file even if they are filed as a Roadmap item.
...
```
| X | Risk | `_check_vix_spike()` reads `engine._vix_spike_active` after `set_vix_spike(True)` — see manager.py L547 |
...
> **Source of truth:** `TRADE_ROADMAP.md` (Completed section). When you ship a strategy item, add a row here in the same commit, following the rules above.
> Last regenerated: 2026-05-07. Items: 164 strategy rows (#1 → #258). Bug-fix rows are intentionally excluded; see `TRADE_ROADMAP.md` for the full Bug Fix log.
...
| 179a | Risk | Per-budget delta on the #179 burst-cap (added 2026-05-06). The 92% lose-together evidence was from a Rs.50K SMALL account, so SMALL/TINY stay at the audit-validated cap-2. NORMAL/LARGE accounts have 5-8 morning slots and the cap-2 single-threads them with no contradicting evidence; they get +1 / +2 deltas (effective cap 3 / 4). Industry parallel: prop-firm risk frameworks tier max-concurrent caps by account size. The bot used to apply one global cap; now it scales with account size while preserving the audit-validated SMALL constraint. |
...
| 251a | Risk | Fractional-Kelly cap on the #251 surviving (un-paused) side (added 2026-05-06; cap raised 3 → 5 on 2026-05-07 after live evidence). When the #251 pause arms against one side, the OPPOSING side may have thin evidence (e.g. SELL n=14 in the original trigger audit; n=10 over the 7-day lookback the day after ship). Per Kelly criterion (typical lookback is 50-60 trades for win-prob estimation; binomial confidence interval at n=14 is ±26 percentage points — statistical noise) the surviving side is capped per session whenever its history has fewer than 20 trades. **Cap retune (2026-05-07):** the original cap of 3 was set on the noisier n=14 evidence; one trading day later the lookback's opposing SELL side (n=10) was running 67% WR (2W/1L) and the cap was the binding rejection on the only profitable side of the day. Cap raised to 5 ≈ MAX_TRADES_PER_DAY/2 on a SMALL account, which still respects fractional-Kelly half-stake-on-uncertain-edge while letting the surviving side capture its hot hand when one is visible. The bot used to lean its full per-session trade quota on the un-paused side just because the OTHER side broke down; now it reduces concentration on the un-validated side while letting it stay alive. |
| 251b | Risk | Intraday NIFTY-bounce bypass on the #251 directional pause (added 2026-05-06). The base #251 gate uses a rolling 7-day NIFTY return to decide whether to arm; in a sustained-bear regime the 7-day stays slightly negative for weeks, so the BUY pause can stay armed indefinitely and the bot collects ZERO fresh BUY evidence — the **BUY-pause loop trap**. New mechanism: each scan, the manager pushes the live NIFTY intraday return into the engine. When two consecutive readings cross +1% (BUY paused) or −1% (SELL paused) in the direction that favours the paused side, the pause-check is bypassed so the bot can probe the apparent regime flip. The pause STATE remains intact for inspection — only the gate-check returns False. Self-limiting: if NIFTY pulls back, the deque drains and the pause re-engages on the next scan. Risk discipline preserved — opposing-thin (#251a), burst-cap (#179/#179a), R:R floor, score floor, RSI cap, ADX gate all still apply. The bot used to be blind to a regime flip while paused; now it can probe one. Industry parallel: directional-change algorithms confirm trend transitions when price crosses a threshold for a sustained confirmation window. |
| 251c | Risk | Tape-breadth divergence bypass on the #251 directional pause (added 2026-05-07). Sister-mechanism to #251b that closes a different blind spot: NIFTY itself can stay flat-to-down on a day when the broader tape (mid-caps, sector leaders) is rallying — NIFTY is cap-weighted so a few mega-caps dominate it. The advance/decline (A/D) line is the textbook industry indicator for exactly this divergence (Investopedia / Wikipedia: "if the index is declining and the A/D line is rising, fewer stocks are declining... the index may be near the end of its decline" — it's a better gauge of average small-to-mid-cap behaviour than the cap-weighted index itself). New mechanism: the scanner already counts BUY-vs-SELL candidates each scan for the #212 tape-breadth penalty; that snapshot is now forwarded to the engine and the directional-pause gate consults it. When the paused side has ≥ 3 candidates (and ≥ 40% of the post-V2_MIN candidate set), the pause-check returns False so the bot can probe the apparent regime flip. The 30-40% band between #212's penalty floor and this bypass floor is an explicit "uncertain — neither rule fires" zone, so the two gates never overlap on the same scan. The pause STATE remains intact for inspection — only the gate-check returns False. Self-limiting: refreshed each scan; if breadth shifts, the gate re-engages on the next scan. Risk discipline preserved — opposing-thin (#251a), burst-cap (#179/#179a), R:R floor, score floor, RSI cap, ADX gate all still apply. Origin: 2026-05-06 BUY pause stayed armed despite mid-cap rally; the dead-code `set_tape_breadth()` API existed but had no caller. The bot used to be blind to A/D divergence; now it has a textbook divergence probe. |
...
| 255 | Risk | Entry-path quote and depth fail-closed on missing data (added 2026-05-07 from the 2026-05-06 phase-2 NoAI audit). Two of the bot's entry-side risk gates — the bid/ask spread gate and the impact-cost gate — used to silently let a trade through whenever the live quote or order-book payload from Zerodha was missing or malformed (the prior "fail-open" behaviour was dating back to the original cautious rollout when missing data was treated as transient). The audit observed that fail-open on missing depth means the bot can take a trade in a name whose order book is genuinely unknown, which is exactly the situation where slippage tail-risk is highest. Same pass tightened the supporting plumbing: every entry-path quote fetch and every scanner pre-filter quote fetch now retries up to three times (with a one-shot session re-login on token-shaped errors and a structured warn-log per failed attempt) before giving up. Net effect: on the happy path — invisible (Kite typically responds on the first or second try); on the rare tail — the bot now refuses to enter a trade when the order book is genuinely unknown after three attempts, instead of silently passing the spread/impact gates with no data. Removal trigger logged as #255R on the roadmap so the change can be reverted if the rejection log shows the new fail-closed behaviour is dropping otherwise-good trades. The bot used to lean on "missing data is probably fine"; now it leans on "missing data after retries means we genuinely don't know." |
| 258 | Risk | Pause score-weighted position sizing in live NoAI (added 2026-05-07; loss-streak intervention). The legacy #107 sizing pass redistributed budget across primary candidates proportional to their absolute technical conviction score (weight = |score| / Σ|scores|, capped at MAX_POSITION_PCT) on the assumption that a score-9 trade deserves more rupees than a score-5 trade. Nine-day live audit (`scripts/trade/analyst_pulse.py`, 2026-04-22 → 2026-05-06, n=55 trades, net −Rs.2,220) showed score-magnitude was **anti-correlated** with realised P&L for every bucket at score ≥ 6: |score| ≥ 9 was −Rs.51 per trade, 8-9 was −Rs.54, 7-8 was −Rs.36, 6-7 was −Rs.43, while the < 6 cohort came in at −Rs.0.28 per trade. Score-weighted sizing was therefore concentrating MORE capital on the worst-performing buckets every single session in the current regime. The bot now defaults to equal sizing (existing fallback path; one slot per primary candidate, sized off budget-per-slot) via the kill-switch `Config.SCORE_WEIGHTED_SIZING_ENABLED = False`; setting it True restores the legacy conviction-weighted behaviour for an A/B revert. Industry rationale: equal-weight (1/N) is the documented out-of-sample benchmark when factor confidence is low — DeMiguel, Garlappi, Uppal (*Review of Financial Studies* 2009) showed that across multiple academic factor models, the 1/N portfolio out-performs parameterised mean-variance allocation precisely BECAUSE estimation error in the factor's edge dominates any in-sample weighting gain. Renaissance / AQR / Two Sigma all DOWN-weight or freeze a factor that loses its predictive R²; they do not double down on it. Re-enable trigger logged on the roadmap as #258R: re-enable when post-#258 ledger shows |score|≥7 cohort outperforming |score|<7 by ≥ +Rs.20/trade over ≥ 30 trades. The bot used to lean its conviction score into per-trade size; now it lets equal sizing run while the scoring model gets a backtest-driven rebuild (#24 / #259 / #265). |
