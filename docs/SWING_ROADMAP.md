...
| S33 | **Per-tier dip% (`SWING_DIP_PCT_BY_INDEX_TIER`).** The current finite-capital evidence is NIFTY 50 only, but the same `SWING_DIP_PCT = 10` is applied to NIFTY 50 large-caps and NIFTY100/150/200 extras. Mid-caps can have wider ATR, thinner liquidity, and deeper drawdowns; a tier-aware cap may keep NIFTY 50 at 10% while requiring a deeper dip for non-NIFTY50 names. | After **≥30 closed dip-buy trades on NIFTY100-extra symbols** (not in NIFTY 50). Compare avg holding-period and net P&L vs the NIFTY-50 cohort; if mid-cap stops fire materially more often than large-cap stops on the same dip threshold, promote and add the per-tier dict. |
...
| S35 | **Pairs / relative-value mean-reversion as a swing setup (`PAIR_REVERSION`).** Carried over from the intraday trade tool (2026-06-01): a market-neutral pairs backtest there was rejected **only because the horizon was wrong** — `scripts/trade/backtest_pairs.py` measured the OU mean-reversion half-life of every viable NIFTY50 same-sector spread at **270–3000+ 15-min bars ≈ 10–40 trading days**. That is *exactly* the swing holding period, so the edge that bled out intraday (forced EOD square-off + two-leg cost daily, OOS PF 0.48–0.69) should be re-tested on **daily candles** where a position can be held until the spread actually converges. Idea: for a frozen list of co-moving same-sector pairs (e.g. INFY/TCS, JSWSTEEL/TATASTEEL, BAJAJFINSV/BAJFINANCE, NTPC/POWERGRID), compute the log-price spread + rolling z-score on daily candles; when |z| ≥ ~2, surface the **cheap leg as a buy candidate** (and, for context, flag the rich leg as a relative-short *watch* only). **Scope honesty:** swing mode is permanently report-only and explicitly bars overnight cash-equity shorts, so the bot must NOT model a real short leg — this ships as a long-only relative-value *timing signal* (buy the lagging leg of a stretched pair), with the short leg shown purely as divergence context. Reuse the frozen TRAIN/TEST split discipline and two-leg-aware logic already proven in `backtest_pairs.py`. | After **S11 swing backtest/replay MVP exists** (needed to validate any new setup on daily candles) AND a daily-candle re-run of the pairs spread shows **OOS PF ≥ 1.15 on the long-only cheap-leg signal net of delivery charges**. If the daily horizon does not clear the gate either, drop the idea permanently — the intraday rejection already rules out the short-horizon version. |
| S36 | **Overnight Return Capture (`OVERNIGHT_HOLD`).** Carried over from intraday research (2026-06-12). Academic evidence (Cliff et al.) shows ~100% of equity returns come from the overnight session globally. Strategy: buy at 3:10 PM close, sell at 9:20 AM next open. Only 1 round-trip/day, avoids intraday noise and cost drag. On NSE: FII overnight positioning + US market direction drives Indian opens. Uses CNC/delivery (not MIS), so no intraday square-off. This is a 1-night swing trade. Can be regime-gated: skip if VIX >20 or NIFTY in downtrend. | After **S11 swing backtest/replay MVP exists** AND a daily-candle backtest of buy-close/sell-open on NIFTY100 shows **OOS PF ≥ 1.15 net of delivery charges (CNC STT)**. Note: CNC STT is 0.1% on sell only (vs MIS 0.025% both sides) so cost structure is different — must be modeled carefully. |
...
| S43 | **AI overlay rendering + persistence + prompt-truncation triple fix.** Origin: 2026-05-14 user reported on a real SBIN AI run: "(a) it printed as raw md on the dashboard - maybe some formatting issue; (b) AI review is not coming up when I go back and enter again the details page; (c) when we go back or enter the swing trade page for the first time it is not showing any list — the last report it should fetch and show right until I dont refresh; (d) I dont see a conclusion line though [in the AI response]". Four fixes shipped: **(A)** `_row_to_candidate()` reconstructed candidates from the FROZEN-AT-SCAN-TIME `snapshot_json` blob and never overlaid live mutable columns on top. The S37 / S38 / manager carry-forward paths all UPDATE the `ai_overlay_json` *column* (not the snapshot blob) — so the detail page's `candidate_by_symbol(sym)` returned a candidate with `ai_overlay_json=""` even when the column had a 2161-byte successful Claude response. Now overlays the live `ai_overlay_json` column on top of the snapshot. Verified: SBIN ai_len went from 0 → 2161 after the fix. **(B)** `latest_run()` / `latest_run_for_date()` / `latest_run_for_date_and_mode()` previously picked the most recent non-snapshot row by `run_id DESC`, so a single-stock SEARCH_BOX run (1 candidate by design — from S38's `analyse_one`) hijacked the dashboard's main recommendations list when the user navigated back after using the search box. The full-scan run with 79 candidates was silently shadowed. Added `AND COALESCE(trigger_source, '') != 'SEARCH_BOX'` to all three queries; SEARCH_BOX results are still queryable per-symbol via `candidate_by_symbol()` so the per-stock detail page still finds them. Verified: dashboard now picks run_id=13 (DASHBOARD_BUTTON, 79 candidates) over run_id=14 (SEARCH_BOX, 1 candidate). **(C)** Dashboard rendered AI overlay text as `<div white-space:pre-wrap>{escape(raw)}</div>` which printed `**bold**` / `## headings` / `---` HRs as literal markdown source. Added `_render_ai_md(text)` server-side helper + `window._aiMdToHtml(text)` JS twin (injected by `_wrap()` so both home + detail pages have it) that renders the four structures Claude actually emits: `**bold**`, `--- HR`, `## H` (1/2/3), `- bullets`, blank-line paragraphs. HTML-escapes input first so Claude can't inject HTML. Used at three call sites: detail-page server render, detail-page button JS callback, search-box result card JS. **(D)** AI prompt rewritten to put VERDICT FIRST (was section 8 → now section 1) so Claude's known truncation tendency at section 6/7 can never cut off the conclusion. The user reported the response stopped at "CORPORATE-ACTION SANITY CHECK" with no verdict — moving it to top makes that impossible. Also: relaxed 400-word cap to 400-600 (the 8-section structure was incompatible with 400 words at the requested depth) and added explicit "ALL 8 SECTIONS ARE MANDATORY" rule. Bonus fix in `latest_ai_overlay_for_symbol()`: previously a stale `{"error":"..."}` payload (57 bytes) on a NEWER run silently shadowed a successful 2161-byte response on an OLDER run during carry-forward. Now skips error-only payloads (defined as `dict with "error" key and no "raw_response" key`) and walks back through up to 8 candidates per symbol. | Indicators | 2026-05-14 |
| S44 | **At-52w-high stocks broken on detail page + Zerodha 2000-day fetch cap + README CLI reference.** Origin: 2026-05-14 user asked "for stocks that are at 52w high like I think BHEL, what does our 52w column show?" + "we can do and read all this from command prompt commands too right? - have them well documented in readme also". Four fixes shipped: **(A)** `core/zerodha_client.py::get_historical()` was passing the full `from_date`..`to_date` range to `kite.historical_data()`. Zerodha caps that endpoint at 2000 days for daily candles (smaller for intraday) — the swing dip-buy scanner asks for ~3650 calendar days (10y) so any wide request silently returned zero bars with `"interval exceeds max limit: 2000 days"`. Verified live: BHEL fetched 0 candles → "Not enough daily history (0 bars; need >=50)" → REJECTED. Now chunks any range > `cap_by_interval[interval]` (1500d for daily, 55d for intraday, 35d for 1-min) into <=cap windows, walks forward with the existing 350ms throttle between calls, dedups boundary overlaps. **(B)** Both `scanner.py` code paths (universe scan + `scan_one`) had `_ath_price = max(_closes_all)` over the FULL history. The dashboard column / detail page rendered the result as "% Below 52w High" but it was actually computing from the all-history close max — semantically wrong, and inconsistent with the dip-buy scanner which uses the `Config.SWING_DIP_LOOKBACK_DAYS = 252` rolling reference. Fixed to slice `_closes_all[-lookback:]` matching the dip-buy scanner's reference. Verified live: BHEL at fresh 52w high now correctly shows `dip_from_ath_pct=0.00%` (was previously the impossible "rejected for no candles"); RELIANCE went from 14.80% → 14.34% (reflecting that part of its dip is from a >1y-old peak); TCS unchanged at 36.46% because its 52w high is also its all-history high. **(C)** Same edit fixed an `UnboundLocalError` in `scan_one()` introduced by S40 — a `from modes.swing.risk import generate_broker_instruction` inside the NONE-fallthrough branch shadowed the module-level binding for the whole function body (Python treats any name assigned anywhere in a function as local for the entire function), breaking the successful-technical-setup path. Removed the redundant local import. **(D)** README §6 gets a new "Swing CLI reference" sub-section documenting all 7 sub-commands (`--actions`, `--positions`, `--confirm <ID> --qty N --price P`, `--skip <ID>`, `--ai`, `--nifty`, `--backtest`) with the same service-layer guarantee that powers the dashboard. Plus a "Read-only inspection from the CLI" block with one-liner Python commands for `latest_run` / `pending_actions` / `open_positions` / `latest_ai_overlay_for_symbol` so any external tooling can drive swing mode end-to-end without the dashboard. Reports under `reports/swing/<YYYY>/<MM>/swing_report_<DD>.txt` (plus the JSON twin) are plain-text and grep-able by design. | Risk | 2026-05-14 |
| S45 | **Compare up to 4 swing stocks side-by-side with sector auto-populate.** Origin: 2026-05-14 user request — "now add a compare feature where I can compare upto 4 stocks. Example I want to know why HDFC is better rated than SBIN. It should also have some auto populated compare example when I say banking sector it auto picks the top 4 banking sector stocks and populate them on the compare ui". Five pieces shipped together: **(A)** New `modes/swing/compare.py` — pure read-only logic. `compare_symbols(symbols, scan_one, sector)` runs the existing `SwingScanner.scan_one()` per symbol, builds a `CompareResult` with one `CompareRow` per metric (status, setup, score, sector, current/52w-high, % below 52w high, above SMA-200/50, above EMA-20, weekly trend up, RSI, RS vs NIFTY, volume ratio, entry/stop/target/qty/RR). Each row carries a `direction` ("high"/"low"/"true"/"rsi"/"neutral") and the dashboard / CLI use it to compute a `winner_idx` per row. `top_n_in_sector(sector, n=4)` reads `SECTOR_MAP` (lazy import to keep cold-path light); `normalise_sector()` aliases free-text input ("bank"/"BANKS"/"banking" → "BANKING", "tech"/"IT" → "IT", "health" → "PHARMA", etc.). Cap of 4 stocks max via `MAX_COMPARE_STOCKS`. **(B)** `GET /api/swing/compare?symbols=A,B,C,D` and `?sector=BANKING` endpoints in `modes/dashboard/server.py`; auth-shaped Zerodha errors surface via the usual `core.error_sink` toast. Plus `GET /api/swing/sectors` for the dropdown. **(C)** New "Compare Stocks (up to 4)" card on `/swing` dashboard between the single-stock search box and the broker-instructions card. Text input + sector dropdown (auto-loads top-4 into the input on change); Compare button POSTs (well, GETs) to the API; result table renders metrics-x-stocks with the winning value highlighted in green per row + a "X wins / Y of Z metrics" tally per stock. Stock symbols in the column headers link to their detail page. **(D)** CLI: `python main.py --mode swing --compare HDFCBANK,SBIN,ICICIBANK,KOTAKBANK` and `python main.py --mode swing --compare-sector BANKING` print a text table with the same data. Verified live for HDFCBANK vs SBIN: "HDFCBANK wins composite score (25.9 vs 20.6) — bigger dip from 52w high — and ties on R:R; SBIN wins on long-term trend (above SMA-200) and volume ratio". For PHARMA sector: "SUNPHARMA wins 5 of 14 metrics, beats DRREDDY on every trend stack". **(E)** README §6 swing CLI reference + the dashboard / API URL mention extended with the new commands. | Indicators | 2026-05-14 |
| S46 | **Per-row Done|Skip → single Add+ button (table-fit + Skip was a no-op).** Origin: 2026-05-14 user feedback — "done and skip button are not looking good on the screen as the table has become big and they don't align side by side anymore, so instead only had one button i.e. done renamed as Add+. Skip has no use as either way this is a readonly advice". Two changes: **(A)** Recommendations table per-row "Done | Skip" pair collapsed to a single bold **Add+** button (with `title="Add this stock to your open swing book — enters qty / price / stop after you fill them on Kite"` tooltip). Same in the search-box result card — "Done — add to swing book | Skip" became "Add+ to swing book | Open detail page". The `confirmAction()` JS path is unchanged: prompts for qty/price/stop, POSTs to `/api/swing/actions/<id>/confirm`, persists the position. **(B)** `skipAction()` JS function deleted (was the only caller of the `/skip` browser path). The server endpoint `/api/swing/actions/<id>/skip` and the `persistence.skip_action()` helper are kept for the CLI `--mode swing --skip <ID>` path, which is still useful for scripting / batch reviews — the dashboard simply no longer offers a button for it because PENDING in a permanently-report-only world is just a "not yet noted" marker; there's no auto-action to suppress, so leaving an action PENDING vs marking it SKIPPED has zero behavioural difference. | Infra | 2026-05-14 |
| S47 | **Dip-buy defaults retuned to finite-capital V2 evidence.** Origin: 2026-05-16 user pointed to newer work in sibling `../market-research/results_v2`. That run changed the assumption set from the earlier infinite-money Rs.10k heatmap to Rs.1L starting capital, Rs.20k lots, sale proceeds recycled, current NIFTY 50, and equal-weight NIFTY 50 benchmark. Result: old `SWING_DIP_PCT=18`, `SWING_DIP_TARGET_PCT=12` ranked only 64/121 (CAGR 18.47%, alpha -1.75%), while X=10/Y=20 ranked #1 (CAGR 21.52%, alpha +1.29%, final value Rs.7.03L). Live defaults are now `SWING_DIP_PCT=10`, `SWING_DIP_TARGET_PCT=20`, `SWING_DIP_BUY_AMOUNT=20000`; fallback defaults, single-stock reasons, dashboard banner, README, SWING_GUIDE, and SWING_STRATEGY were synced. Caveat kept explicit: V2 is still ATH-reference research, while live swing uses the rolling 52w high, so this is provisional until S11 lands a full 52w finite-cap replay. | Indicators | 2026-05-16 |
...
items) have no in-repo backtest evidence base. The standalone
[market-research](https://github.com/yash040599/market-research) repo
now has both the original infinite-money ATH heatmap and the 2026-05-16
finite-capital V2 ATH run; neither validates the live rolling-52w
reference, the technical setups, or the cross-setup modifiers.
