...
#
# Extends PortfolioManager (V1) â€” inherits ALL infrastructure:
...
#   1. StockScannerV2 replaces V1 scanner (math pre-filter â†’ Claude)
#   2. Claude review includes fresh 5-min candle pattern context
#   3. Dynamic poll interval â€” halved when near SL or target (0.5%)
#   4. Periodic candle re-scan every V2_CANDLE_RESCAN_MINUTES (free,
#      no Claude cost) â€” logs strong signals on open positions
#
...
class PortfolioManagerV2(PortfolioManager):
    ...
    pre-filtering. Extends V1 â€” same lifecycle, smarter stock selection.
    """
    ...
    def _print_banner(self):
        ...
        print(f"\n{'='*58}")
        if self._noai:
            print("  AI PORTFOLIO MANAGER â€” V2 NO-AI MODE")
        else:
            print("  AI PORTFOLIO MANAGER â€” V2 CANDLE STRATEGY")
        print(f"{'='*58}")
        if not self._noai:
        ...
        print()
        print(f"  \033[96mâ˜… V2 Strategy\033[0m : Candle patterns + Technical indicators")
        ...
        if self._noai:
            print(f"    AI calls    : ZERO â€” fully rule-based trading")
        print(f"{'='*58}\n")

    # ================================================================
    # TEST MODE â€” run V2 candle pipeline end-to-end, no Claude
    # ================================================================

    def run_test(self, noai: bool = False):
        """
        Runs the full V2 strategy analysis pipeline and shows what the
        bot does at each step â€” without any Claude API calls or orders.
        ...
        print(f"\n{'='*58}")
        print(f"  {mode_label} STRATEGY ANALYSIS â€” TEST MODE")
        ...
        print("  â”‚ 1. Fetch candle data (15-min + daily)           â”‚")
        print("  â”‚ 2. Run 14 candlestick pattern detectors         â”‚")
        print("  â”‚ 3. Compute 12 technical indicators              â”‚")
        print("  â”‚    EMA, RSI, VWAP, SuperTrend, MACD, ORB, Gap,  â”‚")
        print("  â”‚    Daily EMA, Prev-Day S&R, Hourly EMA, BB, ADX â”‚")
        print("  â”‚ 4. Score each stock (~-25 to +25)               â”‚")
        print("  â”‚ 5. Filter by min score threshold                â”‚")
        print("  â”‚ 6. Apply Nifty trend hard filter                â”‚")
        print("  â”‚ 7. Apply sector diversification (max 2/sector)  â”‚")
        if noai:
            print("  â”‚ 8. Auto-select top candidates â†’ BUY/SELL       â”‚")
            print("  â”‚    (score > 0 = BUY, score < 0 = SELL)         â”‚")
        else:
            print("  â”‚ 8. Send top 15 to Claude for final selection    â”‚")
        ...
        # â”€â”€ Step 3: Fetch live quotes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ Step 4: Run V2 pre-filter (math only) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        self.log.section("STEP 2: TECHNICAL ANALYSIS (free â€” no API cost)")
        ...
        # â”€â”€ Step 5: Show ALL results (not just filtered) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ Step 6: Show filtered candidates â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        self.log.section(f"STEP 4: FILTERING â€” {len(filtered)} passed score, {dropped_sector} dropped by sector limit, top {len(top)} shown")
        ...
        # â”€â”€ Step 6b: Nifty hard filter simulation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        if would_drop_bear or would_drop_bull:
            ...
            if would_drop_bear:
                print(f"    BEARISH market â†’ would drop {would_drop_bear} weak BUY signals (score < 3)")
            if would_drop_bull:
                print(f"    BULLISH market â†’ would drop {would_drop_bull} weak SELL signals (score < 3)")

        # â”€â”€ Step 7: Show next step (Claude vs Auto-select) â”€â”€â”€â”€â”€â”€â”€â”€
        if noai:
            ...
            print(f"  â„¹ï¸  In live NoAI mode, ATR-based SL/target would override")
            ...
            if snapshot:
                ...
                print(snapshot)
                print(f"\n  â„¹ï¸  In live V2 mode, Claude would analyse this data and")
        ...
        print(f"\n  Claude calls   : 0  (test mode â€” no API cost)")
        print(f"  Orders placed  : 0  (test mode â€” no trades)")
        if noai:
    ...
    # ================================================================
    # NO-AI MODE â€” fully automated, zero Claude calls
    # ================================================================

    def run_noai(self):
        """
        Runs the full trading day using only technical signals â€”
    ...
    def _run_noai_scan(self, session_context: str = ""):
        """
        Pre-market scan without Claude â€” uses scan_noai() which
        ...
        if now < market_open:
            ...
            self.log.section("MARKET SCAN (NoAI â€” joined late)")
    ...
    # ================================================================
    # OVERRIDE: MONITOR LOOP (V2 â€” with dynamic polling + candle re-scan)
    # ================================================================
    ...
    def _run_monitor_loop(self):
        ...
        """
        if self._noai:
            self.log.section("V2 MONITORING â€” NoAI (rule-based + candle re-scan)")
        else:
            self.log.section("V2 MONITORING â€” Candle-aware price tracking")
        ...
        while not self._shutdown_requested:
            ...
            # â”€â”€ Square-off check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            if self._is_square_off_time(now):
                self._clear_status_line()
                self.log.info("Square-off time reached")
                break

            # â”€â”€ All positions closed? â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            if not self.engine.open_positions():
                if self.engine.is_order_api_broken():
                    self._clear_status_line()
                    self.log.error("All positions closed, order API broken â€” stopping")
                    break
                ...
                if mins_remaining >= self.cfg.MIN_MINUTES_FOR_ENTRY:
                    ...
                    session_ctx = (
                        ...
                        f"  {'If P&L is negative, only pick high-conviction candle setups with tight stops.' if day_pnl < 0 else f'All capital is free â€” deploy at least {self.cfg.MIN_BUDGET_UTILISATION_PCT:.0f}% on high-conviction setups.'}\n"
                    )
                    self._trade_plans = []
                    ...
                    if self._trade_plans:
                        ...
                        self.log.info("V2 re-scan: no new trades â€” done for the day")
                        break
                else:
                    self._clear_status_line()
                    self.log.info(
                        f"All positions closed â€” {mins_remaining:.0f} min left, "
            ...
            try:
                ...
                self.log.warning(f"Quote fetch failed: {e} â€” retrying")
                time.sleep(base_poll)
                continue

            # â”€â”€ SL/target check (free, rule-based) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            if closed > 0:
                ...
                if (
                    ...
                    if mins_left >= self.cfg.MIN_MINUTES_FOR_ENTRY:
                        ...
                        day_pnl = self.engine.day_pnl()
                        session_ctx = (
                            f"\nSESSION CONTEXT (V2 partial re-scan â€” {slots} slot(s) available):\n"
                            f"  Market condition: {self._market_condition}.\n"
                            f"  Day P&L so far: â‚¹{day_pnl:,.2f} from {len(closed_trades)} closed trades.\n"
                            f"  Already traded today: {', '.join(traded_symbols) if traded_symbols else 'none'}.\n"
            ...
            # â”€â”€ Order API broken check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            if self.engine.is_order_api_broken():
                self._clear_status_line()
                self.log.error("Order API broken â€” shutting down")
                if self.engine.open_positions():
                    self._square_off()
                break

            # â”€â”€ Circuit breaker â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            # â”€â”€ Dynamic poll rate â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            if self.engine.open_positions():
                ...
                if near_trigger and not self._fast_poll:
                    ...
                    self._clear_status_line()
                    self.log.info("âš¡ Position near SL/target â€” increasing poll rate")
                elif not near_trigger and self._fast_poll:
                    self._fast_poll = False

            # â”€â”€ Periodic candle re-scan (free, no Claude cost) â”€â”€
            ...
            # â”€â”€ Periodic NIFTY regime re-check (free) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            if nifty_recheck_interval > 0:
                ...
                if nifty_elapsed >= nifty_recheck_interval:
                    ...
                    if self._market_condition and self._market_condition != old_condition:
                        self._clear_status_line()
                        self.log.info(
                            f"ðŸ“Š Market regime shifted: {old_condition} â†’ {self._market_condition}"
                        )
                        # Tighten SLs on positions contradicted by new regime
                        self._regime_shift_protect(quotes)
                    self._last_nifty_check = time.time()

            # â”€â”€ Periodic opportunity scan (fill free slots) â”€â”€â”€â”€â”€â”€â”€
            ...
            if opp_rescan_interval > 0:
                ...
                if (
                    ...
                    if mins_left >= self.cfg.MIN_MINUTES_FOR_ENTRY:
                        self._clear_status_line()
                        self.log.info(
                            f"â° Periodic opportunity scan: {slots} slot(s) free, "
                            f"{mins_left:.0f} min left â€” scanning for new trades..."
                        )
                        self.engine.sync_external_positions()
                        self.engine.refresh_budget()
                        closed_trades = self.engine.closed_positions()
                        ...
                        day_pnl = self.engine.day_pnl()
                        session_ctx = (
                            f"\nSESSION CONTEXT (periodic opportunity scan â€” {slots} slot(s) available):\n"
                            f"  Market condition: {self._market_condition}.\n"
                            f"  Day P&L so far: â‚¹{day_pnl:,.2f} from {len(closed_trades)} closed trades.\n"
                            f"  Already traded today: {', '.join(traded_symbols) if traded_symbols else 'none'}.\n"
                            f"  Currently holding: {', '.join(p['symbol'] for p in self.engine.open_positions())}.\n"
                            f"  You have {slots} slot(s) available. Pick at most {slots} new trade(s).\n"
                            f"  DO NOT pick any stock already traded or currently held.\n"
                            f"  {'If day P&L is negative, only pick high-conviction setups with tight stops.' if day_pnl < 0 else 'Actively look for good setups to fill slots â€” size positions to use capital effectively.'}\n"
                        )
                        self._trade_plans = []
                        self._run_pre_market_scan(session_context=session_ctx)
                        if self._trade_plans:
            ...
            # â”€â”€ Claude review (with candle context) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            # â”€â”€ Print status â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            self._print_status(quotes)

            # â”€â”€ Sleep (dynamic) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _run_claude_review_v2(self, quotes: dict):
        ...
        """
        self.log.section("CLAUDE V2 REVIEW â€” with candle analysis")
    ...
    def _auto_protect_on_contrary_signal(
        ...
        lock in partial profit. This acts immediately â€” no need to
        ...
        if side == "BUY":
            ...
            if profit > 0:
                ...
                # At loss â€” move SL to breakeven (entry)
                new_sl = entry
        ...
        self.log.warning(
            f"âš  CANDLE PROTECT {symbol}: contrary signal (score {score:+.1f}, "
            f"[{patterns}]) â†’ SL tightened â‚¹{old_sl:.2f} â†’ â‚¹{new_sl:.2f}"
        )
    ...
    def _regime_shift_protect(self, quotes: dict):
        ...
        When the Nifty regime flips against open positions, tighten SLs:
          - In profit â†’ lock 50% of profit
          - Near breakeven/loss â†’ move SL to entry (breakeven)
        ...
        for pos in self.engine.open_positions():
            ...
            pos["stop_loss"] = new_sl
            self.log.warning(
                f"âš  REGIME PROTECT {symbol} {side}: market turned {regime} "
                f"â†’ SL tightened â‚¹{old_sl:.2f} â†’ â‚¹{new_sl:.2f}"
            )
