...
::
    ...
    python scripts/trade/strategy_stability_check.py
    python scripts/trade/strategy_stability_check.py --window-days 10 --lookback 60
    python scripts/trade/strategy_stability_check.py --json   # machine-readable
...
SUBSYSTEM_FILES: dict[str, tuple[str, ...]] = {
    "modes/trade/order_engine.py":      ("entry-pipeline", "exit-pipeline", "risk-gates"),
    "modes/trade/stock_scanner.py":  ("scanner-scoring",),
    "shared/technical_indicators.py": ("scanner-scoring",),
    "shared/candle_patterns.py":   ("scanner-scoring",),
    "modes/trade/manager.py":         ("entry-pipeline", "exit-pipeline"),
