...
| D32 | **Responsive fix: wide tables and nav no longer break out of the page.** | MEDIUM | High | Low | ✅ **Shipped 2026-08-10** — three separate causes, all found by measuring `document.scrollWidth` against the viewport at 1440→360px rather than by eye. (1) The 15-column entry-recommendations table is `width:100%` inside a fixed card, so it burst the card instead of scrolling — now wrapped in a shared `.table-scroll` container ([`theme.py`](../theme.py)) with `min-width:1100px`, nowrap cells and a wrapping `.reason` column. (2) `nav.topnav .nav-links` is a flex item defaulting to `min-width:auto`, so it refused to shrink below max-content and alone forced ~500px of page scroll; the breakpoint moved 760→820px and now sets `display:flex; flex-wrap:wrap; min-width:0` (restating `display:flex` because `/swing` predates `nav.py`'s stylesheet and rendered `.nav-links` as a plain block). (3) `/portfolio` sector bars are drawn at `weight_pct × 4` px, so an 87%-weight book drew a 348px bar — a table cell grows to fit an inline-block so CSS `max-width` could not rescue it; clamped in Python via `_bar_px()`. Verified: `/swing`, `/us`, `/portfolio` all report zero horizontal overflow at 1440/1280/1024/820/768/640/480/390/360px. |
| D33 | **`/mf` mutual-fund page — Coin book, externally-held funds, SIPs, NAV history charts.** | HIGH | High | High | ✅ **Shipped 2026-08-10** — [`modes/dashboard/mf_page.py`](../mf_page.py) on top of a new [`modes/mf/`](../../mf/) package. Mutual funds are modelled separately from equities on purpose: an open-ended scheme has **no intraday price**, only an end-of-day NAV, so reusing `StockAnalysis` (52w band / RSI / SMA / beta / sector) would mean a dozen permanently-dead fields and a "live price" that is not live. Every value carries a `NAV as of` chip, and a fund whose NAV cannot be resolved is flagged and held at **cost** rather than marked to zero. Coin holdings / SIPs / orders come from four new `ZerodhaClient` MF methods. Funds held at OTHER brokers are hand-tracked in `data/mf.db` and priced off the Coin catalogue (~7.6k schemes), so the same scheme held in two places merges into one unit-weighted position — the view no single broker statement can give you. Each Coin fetch is snapshotted to disk (`mf_coin_*` tables, replaced wholesale so an exited fund also leaves the local book), so the page and the home net-worth open on the last known book with **no broker call**; a failed live fetch degrades to that snapshot instead of blanking. NAV-history charts join ISIN → AMFI scheme code (parsed from NAVAll.txt, ~18k) → MFapi daily series. Three broker findings worth remembering: Kite's MF `pnl` **and** `xirr` come back `0` for every row (P&L is derived locally; no fake XIRR column was added), and `last_price_date` is always `''` (NAV date backfilled from the catalogue). Net worth on `/` is now Zerodha equity + mutual funds + US — Coin units are not in `kite.holdings()`, so nothing double-counts. CLI parity via `--mode portfolio --type mf`. |
| D34 | **MF structural review + `/us` reframed as a long-term book.** | HIGH | High | Medium | ✅ **Shipped 2026-08-11** — [`modes/mf/insights.py`](../../mf/insights.py), surfaced as a Review section on `/mf` and `--mode portfolio --type mf --insights`. Answers the only question that matters for a 24-fund buy-and-hold book: **how many distinct bets do I own?** Exposure map buckets schemes by what they are actually exposed to (name-based, because Kite's `scheme_type` cannot tell a Nifty-50 tracker from a smallcap one — both are "Index Funds"); pairwise NAV correlation ≥0.90 is the honest stand-in for true holdings overlap, which needs AMFI portfolio disclosures we do not have. Accumulation-vs-dormant split reframes SIPs correctly: **paused SIPs are a deliberate simplification, not a to-do** — the finding is that the dormant corpus will never be diluted by future inflows, so redundancy there only changes if acted on. Consolidation costing prices each merge against the ₹1.25L LTCG exemption, and refuses to claim holding period because Kite's MF order book returns only recent orders. Per-fund CAGR/vol/max-DD come from stored NAV history and **replace** a best/worst-performer widget: since-purchase return mostly measures entry timing, not the fund. Externally-held funds get identical treatment and can record their own monthly SIP so they are not misread as dormant. Also: `/us` is no longer presented as a swing book — it is a long-term portfolio ("US Long-Term Portfolio", "US Holdings", "Long-Term Entry Ideas"); it still shares the swing SQLite schema, but that is storage partitioning, not a holding period. |
| D35 | **`/us` scoring rebuilt for a buy-and-hold horizon.** | HIGH | Highest | High | ✅ **Shipped 2026-08-11** — D34 renamed the US page but left `classify_setup()` (BREAKOUT / PULLBACK / TREND_CONTINUATION / SUPPORT_REVERSAL) driving the ranking, so the label said long-term while the numbers were still swing. That mismatch is fixed: [`modes/us/longterm.py`](../../us/longterm.py) scores six factors with published long-horizon evidence — quality & profitability 24 (Novy-Marx gross profitability), valuation vs **sector median** 18 (an absolute P/E cannot say why software at 30x and a bank at 12x are both fair), growth durability 17, 12-1 momentum 16 (Jegadeesh-Titman, plus a 200-DMA filter to avoid value traps), financial strength 13, risk & drawdown 12 (Frazzini-Pedersen BAB). ATR stops, R-multiples and price targets are **removed** from the US surface, not relabelled: they assume a planned exit in weeks. Ratings are HIGH CONVICTION / ACCUMULATE / NEUTRAL / WEAK / AVOID; uncovered pillars are dropped and re-weighted, and coverage under 55% caps the rating so a two-field model cannot read as conviction. Holdings table swapped Stop/Target/R for Weight + current Rating, so a decayed business surfaces in the book rather than in a stop-loss. Fundamentals cached 14 days in `data/us_fundamentals.json`. **Data bug found and fixed on the way:** yfinance's `info["freeCashflow"]` reported 16.5bn for MSFT against a filed 67bn — a 4x understatement that fed 26% of the valuation pillar; FCF is now derived from the cash-flow statement. 19 tests in `tests/test_us_longterm.py`, including an assertion that swing vocabulary can never leak back into the action map. |
| D36 | **Whole-book Ask-AI prompt and dashboard usability pass.** | MEDIUM | High | Medium | ✅ **Shipped 2026-08-12** — the home dashboard now builds a copy-paste prompt containing the private India-equity, mutual-fund and US snapshot, allocation, SIP flow and duplicate-exposure context, with row caps for free-model context windows. The prompt stays local until the user copies it; the dashboard never sends it to an LLM or places orders. Home and `/us` share one persistent USD/INR display preference. `/mf` now gives visible refresh progress/completion feedback and a fund-selectable NAV chart whose selection is restored after the book rerenders. |
