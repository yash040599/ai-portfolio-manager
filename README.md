...
2. **Execute** — LIMIT entry at LTP + 1 tick (MARKET fallback), ATR-based
   SL/target with min-distance floor, **38-check pre-trade pipeline** (see
...
| Doc | What it covers |
|-----|----------------|
| [docs/STRATEGY_V2.md](docs/STRATEGY_V2.md) | Complete V2 strategy — NoAI + AI modes, 38-check pre-trade pipeline, all indicators/patterns, scoring, risk layers, glossary |
...
│   ├── order_engine.py           # 38-check entry pipeline + monitoring
